...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Target_Brain"))
...
TARGET_PATH     = os.path.join(BASE_DIR, "..", "Target_Brain", "target_ppo")

print("=" * 58)
print("  ARENA TEST")
print("  SAC Missile (Phase 3d)  vs  PPO Target")
print("  Both brains FROZEN — zero-shot adversarial test")
print("=" * 58)

missile_model = SAC.load(MISSILE_PATH)
target_model  = PPO.load(TARGET_PATH)
...
v_T_max  = TargetEnv.v_T_max
v_I      = 250.0
v_I_max  = 250.0
r_kill   = MissileEnvPhase3b.r_kill      # 300m
r_kill_I = MissileEnvPhase3b.r_kill_I   # 150m
max_steps = MissileEnvPhase3b.max_steps

# Interceptor spawn distance
int_spawn_min = 800.0

episodes = 100
missile_hits = target_survived = intercepted = oob = timeout = 0
...
for ep in range(episodes):
    ...
    while True:
        ...
        if np.sqrt((tx-mx)**2+(ty-my)**2+(tz-mz)**2) >= 300: break

    tv   = rng.uniform(TargetEnv.v_T_min, TargetEnv.v_T_max)
    ...
    while True:
        ...
        if np.sqrt((i1x-mx)**2+(i1y-my)**2+(i1z-mz)**2) >= int_spawn_min: break
    while True:
        ...
        if d_m >= int_spawn_min and d_12 >= 500: break

    i1vx=i1vy=i1vz=i2vx=i2vy=i2vz=0.0
    i1_active = i2_active = True

    prev_d_MT = float(np.sqrt((tx-mx)**2+(ty-my)**2+(tz-mz)**2))
    outcome   = "timeout"
    ...
    for step in range(max_steps):
        ...
        # ── Build missile observation (22-dim, same as Phase 3b/3d) ──
        ...
        if i1_active:
            ...
            d_MI1=float(np.sqrt(ix**2+iy**2+iz**2)); phi_MI1=wrap(np.arctan2(iy,ix)-mtheta)
        ...
        if i2_active:
            ...
            d_MI2=float(np.sqrt(ix**2+iy**2+iz**2)); phi_MI2=wrap(np.arctan2(iy,ix)-mtheta)
        ...
        missile_obs = np.array([
            ...
            0.0, 0.0, 0.0,   # target velocity — missile doesn't know target is smart
            d_MI1/D_3D, phi_MI1/np.pi, elev_MI1/(np.pi/2), n_i1vx, n_i1vy, n_i1vz,
            d_MI2/D_3D, phi_MI2/np.pi, elev_MI2/(np.pi/2), n_i2vx, n_i2vy, n_i2vz,
        ], dtype=np.float32)

        # ── Build target observation (10-dim) ──
        dx2=mx-tx; dy2=my-ty; dz2=mz-tz
        d_TM    = float(np.sqrt(dx2**2+dy2**2+dz2**2))
        phi_TM  = wrap(np.arctan2(dy2,dx2) - ttheta)
        elev_TM = float(np.arctan2(dz2, np.hypot(dx2,dy2)))
        mvx_now = mv*np.cos(mgamma)*np.cos(mtheta)
        mvy_now = mv*np.cos(mgamma)*np.sin(mtheta)
        mvz_now = mv*np.sin(mgamma)

        target_obs = np.array([
            d_TM/D_3D, phi_TM/np.pi, elev_TM/(np.pi/2),
            tv/v_T_max, ttheta/np.pi, tgamma/(np.pi/2),
            mvx_now/v_M_max, mvy_now/v_M_max, mvz_now/v_M_max,
            d_TM/D_3D,
        ], dtype=np.float32)

        # ── Get actions from both brains ──
        m_action, _ = missile_model.predict(missile_obs, deterministic=True)
        t_action, _ = target_model.predict(target_obs,  deterministic=True)
        ...
        # ── Apply target action (PPO brain) ──
        a_yaw_t  = float(t_action[0]) * TargetEnv.omega_max
        a_pitch_t= float(t_action[1]) * TargetEnv.pitch_max
        dv_t     = float(t_action[2]) * 20.0
        tv     = float(np.clip(tv + dv_t*dt, TargetEnv.v_T_min, TargetEnv.v_T_max))
        ...
        for i in [1, 2]:
            ...
            ix_=(i1x if i==1 else i2x); iy_=(i1y if i==1 else i2y); iz_=(i1z if i==1 else i2z)
        ...
        for active,ix_,iy_,iz_ in [(i1_active,i1x,i1y,i1z),(i2_active,i2x,i2y,i2z)]:
        ...
        if tx<0 or tx>L or ty<0 or ty>L or tz<0 or tz>H:
            outcome = "target_oob"; break

        prev_d_MT = current_d_MT

    if outcome == "missile_hit":   missile_hits    += 1
    elif outcome == "intercepted": intercepted     += 1
    elif outcome == "oob":         oob             += 1
    elif outcome == "timeout":     target_survived += 1
...
print()
print("=" * 55)
print("  ARENA RESULTS — SAC Missile vs PPO Target")
print("=" * 55)
...
print(f"  Target  win rate : {target_survived + intercepted}%")
print("=" * 55)
