...
# ------------------------------------------------------------------ #
#  Load model — full difficulty                                        #
# ------------------------------------------------------------------ #
MissileEnvPhase3b.v_I           = 250.0
MissileEnvPhase3b.int_spawn_min = 800.0

model = SAC.load(os.path.join(BASE_DIR, "missile_sac_phase3b"))
    ...
    while True:
        ...
        for trail, ax, ay, az, active in [
            ...
            if active:
                ...
                trail["x"].append(trail["x"][-1])
                trail["y"].append(trail["y"][-1])
                trail["z"].append(trail["z"][-1])
        ...
        d_MI1 = float(np.sqrt(
            (env.int1_x - env.x)**2 + (env.int1_y - env.y)**2 + (env.int1_z - env.z)**2
        )) if env.int1_active else 9999.0

        d_MI2 = float(np.sqrt(
            (env.int2_x - env.x)**2 + (env.int2_y - env.y)**2 + (env.int2_z - env.z)**2
        )) if env.int2_active else 9999.0
...
gs = gridspec.GridSpec(
    6, 2, figure=fig,
    width_ratios=[2.2, 1],
    height_ratios=[1.1, 1.1, 1.1, 1.1, 0.9, 0.9],
...
engage_text = fig.text(
    0.02, 0.02, "", color="white", fontsize=8, va="bottom", fontfamily="monospace",
...
outcome_text = fig.text(
    ...
    fontweight="bold", ha="center", va="center", fontfamily="monospace", alpha=0.0,
    bbox=dict(boxstyle="round,pad=0.5", facecolor="#000000", alpha=0.0)
)

mis_text = ax_mis.text(0.04, 0.96, "initializing...", transform=ax_mis.transAxes,
    color="#00ffcc", fontsize=7.5, va="top", fontfamily="monospace")
tgt_text = ax_tgt.text(0.04, 0.96, "initializing...", transform=ax_tgt.transAxes,
    color="#ff9900", fontsize=7.5, va="top", fontfamily="monospace")
i1_text  = ax_i1.text(0.04, 0.96, "initializing...", transform=ax_i1.transAxes,
    color="#ff2255", fontsize=7.5, va="top", fontfamily="monospace")
i2_text  = ax_i2.text(0.04, 0.96, "initializing...", transform=ax_i2.transAxes,
    color="#ff66aa", fontsize=7.5, va="top", fontfamily="monospace")
...
def _update_charts(ep_idx):
    ...
    hr_y = [sum(1 for j in done[:k+1] if episodes_data[j]["outcome"] == "hit") / (k+1) * 100
...
def _missile_hud(f):
    ...
    return (
        ...
        f"  TGT   {f['d_MT']:6.0f} m\n"
        f"  INT1  {f['d_MI1']:6.0f} m\n"
        f"  INT2  {f['d_MI2']:6.0f} m\n"
        f"  CLOS  {c}{abs(f['closing_speed']):5.1f} m/s"
    )
...
def _int_hud(f, n):
    active = f[f"int{n}_active"]
    spd    = f[f"int{n}_speed"]
    iz     = f[f"i{n}z"]
    vx     = f[f"int{n}_vx"]
    vy     = f[f"int{n}_vy"]
    vz     = f[f"int{n}_vz"]
...
def update(gi):
    ...
    if kind == "play":
        ...
        if f["int1_active"]:
            _set_trail(old_ti1, mid_ti1, new_ti1, f["i1x_trail"], f["i1y_trail"], f["i1z_trail"])
        else:
            for ln in [old_ti1, mid_ti1, new_ti1]: ln.set_data([], []); ln.set_3d_properties([])

        if f["int2_active"]:
            _set_trail(old_ti2, mid_ti2, new_ti2, f["i2x_trail"], f["i2y_trail"], f["i2z_trail"])
        else:
            for ln in [old_ti2, mid_ti2, new_ti2]: ln.set_data([], []); ln.set_3d_properties([])

        missile_dot.set_data([f["mx"]], [f["my"]]); missile_dot.set_3d_properties([f["mz"]])
        missile_dot.set_color("#00ffcc"); missile_dot.set_markersize(9)
        target_dot.set_data([f["tx"]], [f["ty"]]);  target_dot.set_3d_properties([f["tz"]])
        target_dot.set_color("#ff9900"); target_dot.set_markersize(14)

        if f["int1_active"]:
            int1_dot.set_data([f["i1x"]], [f["i1y"]]); int1_dot.set_3d_properties([f["i1z"]])
        else:
            int1_dot.set_data([], []); int1_dot.set_3d_properties([])

        if f["int2_active"]:
            int2_dot.set_data([f["i2x"]], [f["i2y"]]); int2_dot.set_3d_properties([f["i2z"]])
        else:
            int2_dot.set_data([], []); int2_dot.set_3d_properties([])
        ...
        i1_text.set_text(_int_hud(f, 1))
        i2_text.set_text(_int_hud(f, 2))
        engage_text.set_text(_engage_hud(f, ep_idx))
        ax3d.set_title(f"Phase 3b  —  Episode {ep_idx+1} / {NUM_EPISODES}",
        ...
        if outcome == "hit":
            ...
            missile_dot.set_data([], []); missile_dot.set_3d_properties([])
    ...
    return (old_tm, mid_tm, new_tm, old_tt, mid_tt, new_tt,
            ...
            missile_dot, target_dot, int1_dot, int2_dot,
            explosion, int_kill,
            mis_text, tgt_text, i1_text, i2_text,
            engage_text, outcome_text, hit_rate_line, *bar_rects)
